...
class ToolCommandManager:
    ...
    def validate_commands(commands: list) -> list:
        blocked = []
        allowed_prefixes = ("show ", "terminal length", "terminal width", "dir", "pwd")
        dangerous_prefixes = (
...
class CommandPolicy:
    SAFE_PREFIXES = ("show ", "terminal length ", "terminal width ", "dir ", "pwd ")
...
class ConnectionManager:
    ...
    def connect(ip: str, creds: dict, platform_choice: str, temp_session_log: str):
        try:
            ...
            try:
                ...
                return ConnectionResult(host=ip, status=ConnectionStatus.AUTH_FAILED, error_message=str(e))
            except NetmikoTimeoutException as e:
                return ConnectionResult(host=ip, status=ConnectionStatus.TIMEOUT, error_message=str(e))
            except Exception as e:
                err = str(e).lower()
                ...
                return ConnectionResult(host=ip, status=status, error_message=str(e))
    ...
    @staticmethod
    def connect_with_mapped_or_global_credentials(host: str, platform_choice: str, temp_session_log: str, credential_store, mapping_store, log_callback, stop_event=None, fallback_to_all=False):
        if mapping_store is None:
            return ConnectionManager.connect_with_global_credentials(host, platform_choice, temp_session_log, credential_store.as_netmiko_dicts(), log_callback, stop_event)
        ...
        res = ConnectionManager.connect(host, c_dict, platform_choice, temp_session_log)
        ...
        if fallback_to_all:
            ...
            for r in remaining_creds:
                ...
                rem_res = ConnectionManager.connect(host, rem_dict, platform_choice, temp_session_log)
    ...
    @staticmethod
    def connect_with_global_credentials(host: str, platform_choice: str, temp_session_log: str, credentials: List[Dict], log_callback, stop_event=None) -> ConnectionResult:
        attempt_history = []
        last_result = None
        ...
        for i, cred in enumerate(credentials, 1):
            ...
            res = ConnectionManager.connect(host, cred, platform_choice, temp_session_log)
                ...
                with open(outfile, "w", encoding="utf-8") as f:
                    ...
                    def exec_cmd(cmd):
                        ...
                        try:
                            ...
                            self.enqueue("LOG_EXEC", f"  ✗ Error running {cmd}: {str(e)}")
                            f.write(f"\n## {cmd}\nERROR: {str(e)}\n")
    ...
    def execution_thread(self, config: ScannerRunConfig):
        try:
            ...
            for idx, host in enumerate(config.targets, 1):
                ...
                def log_cb(msg):
                    self.enqueue("LOG_EXEC", msg)
                conn_result = ConnectionManager.connect_with_mapped_or_global_credentials(host, config.platform_choice, temp_session_log, self.controller.credential_store, getattr(self.controller, 'target_credential_store', None), log_cb, self.stop_event, getattr(self, 'fallback_to_all_credentials_for_run', False))
                ...
                if not cmd_bundle:
                    ...
                    for cmd in cmd_bundle:
                        ...
                        try:
                            ...
                            self.enqueue("LOG_EXEC", f"  ✗ Error: {cmd} - {str(e)}")
                            outputs[cmd] = f"ERROR: {str(e)}"
                            errors.append(f"Error on {cmd}: {str(e)}")
...
class CredentialMappingRunner:
    ...
    def map_targets(targets, credential_store, mapping_store, platform_choice, 
                    ui_callbacks, stop_event, retest_mapped=False, only_unmapped_or_stale=True, capture_mode="redacted"):
        ...
        for idx, host in enumerate(targets):
            ...
            for i, cred_record in enumerate(creds_to_try, 1):
                ...
                res = ConnectionManager.connect(host, cred_dict, platform_choice, temp_session_log)
